...
public abstract class GridController implements IDataObject
{
     ...
     */
    private WorldPos referenceCoord;
    ...
    protected GridController (World world)
    {
        // Multiblock stuff
        worldObj = world;
    ...
    public void attachBlock (AbstractMultiblockNode part)
    {
        ...
        {
            Settings.LOGGER.warn("[%s] Controller %s is double-adding part %d @ %s. This is unusual. If you encounter odd behavior, please tear down the machine and rebuild it.", (worldObj.isRemote ? "CLIENT" : "SERVER"), hashCode(), part.hashCode(), coord);
        }
        ...
        if (minimumCoord != null)
        {
            ...
            {
                minimumCoord.setX(part.xCoord);
            }
            if (part.yCoord < minimumCoord.y())
            {
                minimumCoord.setY(part.yCoord);
            }
            if (part.zCoord < minimumCoord.z())
            {
                minimumCoord.setZ(part.zCoord);
            }
        }
        ...
        if (maximumCoord != null)
        {
            ...
            {
                maximumCoord.setX(part.xCoord);
            }
            if (part.yCoord > maximumCoord.y())
            {
                maximumCoord.setY(part.yCoord);
            }
            if (part.zCoord > maximumCoord.z())
            {
                maximumCoord.setZ(part.zCoord);
            }
        }

        MultiblockRegistry.instance().addDirtyController(worldObj, this);
    }
    ...
    public void detachBlock (AbstractMultiblockNode part, boolean chunkUnloading)
    {
        ...
        {
            Settings.LOGGER.warn("[%s] Double-removing part (%d) @ %d, %d, %d, this is unexpected and may cause problems. If you encounter anomalies, please tear down the reactor and rebuild it.", worldObj.isRemote ? "CLIENT" : "SERVER", part.hashCode(), part.xCoord, part.yCoord, part.zCoord);
        }

        if (connectedParts.isEmpty())
        {
            // Destroy/unregister
            MultiblockRegistry.instance().addDeadController(this.worldObj, this);
            return;
        }

        MultiblockRegistry.instance().addDirtyController(this.worldObj, this);
     ...
     */
    public void assimilate (GridController other)
    {
        WorldPos otherReferenceCoord = other.getReferenceCoord();
     ...
     */
    private void _onAssimilated (GridController otherController)
    {
        if (referenceCoord != null)
        {
     ...
     */
    protected abstract void onAssimilate (GridController assimilated);
     ...
     */
    protected abstract void onAssimilated (GridController assimilator);
     ...
     */
    public WorldPos getReferenceCoord ()
    {
        if (referenceCoord == null)
        {
            selectNewReferenceCoord();
     ...
     */
    public WorldPos getMinimumCoord ()
    {
        if (minimumCoord == null)
        {
            recalculateMinMaxCoords();
        }
        return minimumCoord.clone();
    }

    /**
     * @return The maximum bounding-box coordinate containing this machine's blocks.
     */
    public WorldPos getMaximumCoord ()
    {
        if (maximumCoord == null)
        {
            recalculateMinMaxCoords();
        }
        return maximumCoord.clone();
    }
     ...
     */
    public boolean shouldConsume (GridController otherController)
    {
        ...
        else
        {
            ...
            Settings.LOGGER.warn("[%s] Encountered two controllers with the same reference coordinate. Auditing connected parts and retrying.", worldObj.isRemote ? "CLIENT" : "SERVER");
            auditParts();
            ...
            {
                Settings.LOGGER.fatal("My Controller (%d): size (%d), parts: %s", hashCode(), connectedParts.size(), getPartsListString());
                Settings.LOGGER.fatal("Other Controller (%d): size (%d), coords: %s", otherController.hashCode(), otherController.connectedParts.size(), otherController.getPartsListString());
    ...
    private int _shouldConsume (GridController otherController)
    {
        WorldPos myCoord = getReferenceCoord();
        WorldPos theirCoord = otherController.getReferenceCoord();
    ...
    private void auditParts ()
    {
        ...
        Settings.LOGGER.warn("[%s] Controller found %d dead parts during an audit, %d parts remain attached", worldObj.isRemote ? "CLIENT" : "SERVER", deadParts.size(), connectedParts.size());
    }
    ...
    public Set<AbstractMultiblockNode> checkForDisconnections ()
    {
        ...
        if (this.isEmpty())
        {
            MultiblockRegistry.instance().addDeadController(worldObj, this);
            return null;
        }
    ...
    private void selectNewReferenceCoord ()
    {
        ...
        for (AbstractMultiblockNode part : connectedParts)
        {
            ...
            if (referenceCoord == null || referenceCoord.compareTo(new WorldPos(worldObj, part.xCoord, part.yCoord, part.zCoord)) > 0)
            {
